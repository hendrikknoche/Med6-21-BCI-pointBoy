...
ggplot(resbyMeasureByCondition,aes(x=Measure,y=mean, colour=Condition))+geom_point(position = position_dodge(width=0.25))+geom_errorbar(aes(ymin=lci,ymax=uci),position = "dodge", width = 0.25)+coord_flip()
